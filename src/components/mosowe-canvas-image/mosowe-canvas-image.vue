  ...
  created () {
    uni.getSystemInfo({
      success:(sysInfo)=> {
        ...
        //ios或开发者工具
        if(/ios/i.test(sysInfo.platform)||sysInfo.platform==="devtools"){
          this.scaleRatio=uni.getSystemInfoSync().windowWidth / 375
        }else{//android
          this.scaleRatio=(uni.getSystemInfoSync().windowWidth / 375)*0.85
        }
      }
    });
        ...
        if (item.content.indexOf('https://') > -1) { // https://网络图片
          ...
          if (this.compress && item.hasOwnProperty('file') && item.file.size > this.compressSize) { // 大于限制2M压缩
            ...
            if (item.arc) {
              ...
              this.drawBgImage(item);
            }
          }
        }
			}
			else if (item.type === 'image') { // 图片
				if (item.content.indexOf('https://') > -1) { // https://网络图片
					// #ifndef H5
					// 非H5
					this.downloadImageNotH5(item);
    ...
    drawBgImage (item) {
      ...
      this.checkDrawOver();
		...
		drawImage (item) {
          ...
          this.remSize(item.x),
          this.remSize(item.y),
					item.hasOwnProperty('width') ? this.remSize(item.width) : this.width,
					item.hasOwnProperty('height') ? this.remSize(item.height) : this.height
				);
				this.checkDrawOver();
		},
		// 文本绘制
		drawText (item) {
				...
				this.canvas.setFontSize(item.hasOwnProperty('size')? this.remSize(item.size) : 20);
				this.canvas.setTextAlign(item.hasOwnProperty('align') ? item.align: 'left');
				this.canvas.globalAlpha = item.hasOwnProperty('globalAlpha') ? item.globalAlpha : 1;

				if (item.maxWidth) {
					this.canvas.fillText(item.content, this.remSize(item.x),this.remSize(item.y), item.maxWidth);
				} else {
					this.canvas.fillText(item.content, this.remSize(item.x), this.remSize(item.y));
				}
				this.checkDrawOver();
		},
